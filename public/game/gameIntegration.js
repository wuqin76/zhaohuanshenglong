...
(function() {
    ...
    function waitForDependencies(callback) {
        ...
        const checkInterval = setInterval(() => {
            ...
            if (attempts % 10 === 0) {
                console.log(`⏳ 等待依赖加载... (${attempts/10}秒) FishCounter:${hasFishCounter} cc:${hasCC}`);
            }
            
            // 只需要 FishCounter 和 cc 即可，director 会在游戏启动后创建
            if (hasFishCounter && hasCC) {
        ...
        setTimeout(() => {
            clearInterval(checkInterval);
            const hasMinimal = window.FishCounter && window.cc;
            if (hasMinimal) {
                console.log('✅ 超时后检测到基础依赖已加载，开始集成');
                callback();
            } else {
